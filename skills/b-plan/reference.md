...
Before using this skeleton, read `${CLAUDE_SKILL_DIR}/references/b-agentic/contract/02-source-of-truth.md` and `${CLAUDE_SKILL_DIR}/references/b-agentic/contract/08-artifacts.md`. New saved plans include durable frontmatter before the Markdown body. Use the path and slug conventions to save the file with an English `<plan-file-slug>` filename, while frontmatter `slug` stays the canonical `<task-slug>`.
...
**Scope:** <files or area>
**Risk:** <trivial | low>   (after reading `${CLAUDE_SKILL_DIR}/references/b-agentic/contract/03-definitions.md`)
...
- **Revise in place** when the goal, touch points, and most steps survive. Read `${CLAUDE_SKILL_DIR}/references/b-agentic/contract/02-source-of-truth.md` before applying the revision protocol.
