...
In this source repo, shared runtime rules live in `global/AGENTS.md` and install to `~/.config/opencode/AGENTS.b-skills.md`; the installer replaces the active `AGENTS.md` only when missing or approved. Installed skills still cite `AGENTS.md`, so preserved third-party rules leave the suite activation-pending until merged/replaced. The global file is intentionally kept as the hot-path runtime contract: concise shared rules stay there, while longer playbook detail belongs in skill or reference files.

Runtime headlines: routing and trigger precedence (§1), source-of-truth and saved-plan lifecycle (§2), definitions and rubrics (§3), MCP bundles and fallbacks (§4), evidence standards and citation provenance (§5), safety gates (§6), execution and verification discipline (§7), artifacts (§8), output schemas and verbosity limits (§9), cross-cutting decisions such as the high-risk challenge gate and test-vs-bug routing (§10), session lifecycle (§11), and the suite-wide anti-pattern table (§12).
